...
(defun gen-columns-list (editor)
  (cons (list :title "name" :adjust :left :default-width 300)
        (mapcar #'(lambda (item) 
              (list :title (string-downcase item) :adjust :left :default-width 50))
          (cdr (elements-display editor)))))

(defun gen-column-elements (element editor)  
  (mapcar #'(lambda (item) 
  ...
  (if *current-workspace*
      (let* ((ed (editor *current-workspace*))
        ...
        (let ((elements-list (om-make-di 'om-multicol-item-list
                                         :columns (gen-columns-list ed)
                                         :items (filter-ws-elements (elements ws) (elements-view-filter ed))
                                         :column-function #'(lambda (item) (gen-column-elements item ed))
        ...
        (om-make-layout 
         'om-column-layout :name "Workspace Elements" :align :center :ratios '(1 nil 1)
         :subviews (list nil 
                         (om-make-di 'om-multi-text :enable nil :size (om-make-point 300 60) 
