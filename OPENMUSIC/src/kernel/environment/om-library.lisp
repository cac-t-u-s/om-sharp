...
(defun register-om-library (path)
  (let ((new-library (make-new-om-library path)))
    (if new-library
        (if (find-om-library (name new-library))
            (om-beep-msg "A library named ~A already exists. Could not register library." (name new-library))
  ...
  (let ((packager-loader (lib-loader-file lib)))                                      
    (if (probe-file packager-loader)
      (handler-bind ((error #'(lambda (c)
                                ...
                                (om-abort))))
        (let ((*current-lib* lib))
          (om-print-format "Loading library: ~A..." packager-loader "OM")
          (load packager-loader)
          (setf (loaded? lib) t)
          (om-print-format "Loading library: Done." nil "OM")
          packager-loader))
      (om-beep-msg "Library doesn't have a loader file: ~A.lisp not found.." lib-name))
    ))
