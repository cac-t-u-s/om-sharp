...
(defun SdifGenInit (str) (with-foreign-string (cstr str) (sdif::SdifGenInitSTR cstr)))

(defcfun  ("SdifGenInitCond" SdifGenInitCondSTR) :void (PredefinedTypesFile sdif-name))
(defun SdifGenInitCond (str) (with-foreign-string (cstr str) (sdif::SdifGenInitCondSTR cstr)))
...
(defun SdifFOpen (str mode) (with-foreign-string (cstr str) (sdif::SdifFOpenSTR cstr mode)))

(defcfun  ("SdifFClose" SdifFClose) :void (sdifF :pointer))

(defcfun  ("SdifCheckFileFormat" SdifCheckFileFormatSTR) :int  (name sdif-name))
(defun SdifCheckFileFormat (str) (with-foreign-string (cstr str) (sdif::SdifCheckFileFormatSTR cstr)))

(defcfun  ("SdifToText" SdifToTextSTR) :unsigned-int (sdifF :pointer) (textStreamName sdif-name))
(defun SdifToText (sdiff str) (with-foreign-string (cstr str) (sdif::SdifToTextSTR sdiff cstr)))
...
(defun SdifSignatureToString (sdifsignature) (foreign-string-to-lisp (sdif::SdifSignatureToStringSTR sdifsignature)))

(defcfun ("SdifStringToSignature" SdifStringToSignatureSTR) :pointer (str :pointer))
(defun SdifStringToSignature (str) (with-foreign-string (cstr str) (sdif::SdifStringToSignatureSTR cstr)))
...
;;; !!! actually does not return INT but SdifDataTypeET ...
...
(defun SdifNameValuesLGet (nvl name) (with-foreign-string (strname name) (sdif::SdifNameValuesLGetSTR nvl strname)))
...
(defun SdifNameValueGetName (nv) (foreign-string-to-lisp (sdif::SdifNameValueGetNameSTR nv)))

(defcfun  ("SdifNameValueGetValue" SdifNameValueGetValueSTR) :pointer (NV :pointer))
(defun SdifNameValueGetValue (nv) (foreign-string-to-lisp (sdif::SdifNameValueGetValueSTR nv)))
...
(defun SdifStringAppend (ptr str)
  (with-foreign-string (strp str)
    (sdif::SdifStringAppendSTR ptr strp)))
