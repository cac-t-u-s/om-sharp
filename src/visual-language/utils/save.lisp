...
(defmethod om-load-from-id ((id (eql :in)) data)
  (let* ((type (find-value-in-kv-list data :type))
    ...
    (if (find-class type nil)
        (let ((in (make-instance type :name name :defval (omng-load defval) :doc doc)))
          (setf (index in) index)
          in)
      (om-beep-msg "Warning: Input of type ~A could not be loaded (unknown type)." type))
    ))
...
(defmethod om-load-from-id ((id (eql :out)) data)
  (let* ((type (find-value-in-kv-list data :type))
         ...
         (doc (find-value-in-kv-list data :doc)))
    (if (find-class type nil)
        (let ((out (make-instance type :name name :doc doc)))
