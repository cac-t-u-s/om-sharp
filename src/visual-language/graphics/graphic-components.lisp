...
(defclass om-icon-button (om-item-view)
  ((icon :initform nil :accessor icon :initarg :icon)
   ...
   (pict-pushed :initform nil :accessor pict-pushed)
   (pict-disabled :initform nil :accessor pict-disabled)
...
(defmethod om-draw-contents ((self om-icon-button))
  ...
  (when (and (icon self) (not (pict self)))
    (setf (pict self) (om-load-picture-for-view (icon self) self)))
  (when (and (icon-pushed self) (not (pict-pushed self)))
    (setf (pict-pushed self) (om-load-picture-for-view (icon-pushed self) self)))
  (when (and (icon-disabled self) (not (pict-disabled self)))
    (setf (pict-disabled self) (om-load-picture-for-view (icon-disabled self) self)))

  (let* ((icn (or (and (pushed self) (icon-pushed self) (pict-pushed self))
                  (and (not (enabled self)) (icon-disabled self) (pict-disabled self))
                  (and (icon self) (pict self)))))
